...
def randomuserAgent():
    ...
    ADID = ''.join(random.sample('0987654321ABCDEF', 8)) + '-' + ''.join(random.sample('0987654321ABCDEF', 4)) + '-' + ''.join(random.sample('0987654321ABCDEF', 4)) + '-' + ''.join(random.sample('0987654321ABCDEF', 4)) + '-' + ''.join(
        random.sample('0987654321ABCDEF', 12))
...
def getcookie_wskey(key):
    ...
    for num in range(0, 20):
        ...
        headers = {"cookie": key, 'user-agent': UserAgent, 'accept-language': 'zh-Hans-CN;q=1, en-CN;q=0.9', 'content-type': 'application/x-www-form-urlencoded;'}
        try:
...
def subcookie(pt_pin, cookie, token):
    if True:
        ...
        if token != "":
            ...
            body = {'searchValue': pt_pin, 'Authorization': f'Bearer {token}'}
...
def main():
    ...
    if envtype == "ql":
        ...
        url = 'http://127.0.0.1:5600/api/envs'
        headers = {'Authorization': f'Bearer {token}'}
        body = {'searchValue': 'JD_WSCK', 'Authorization': f'Bearer {token}'}
        datas = get(url, params=body, headers=headers).json()['data']
    elif envtype == "HW":
        with open(config, "r", encoding="utf-8") as f1:
            #token = json.load(f1)['token']
    ...
    if envtype == "ql":
        ...
        for ws, remark in wslist:
            ...
            pinnum += 1
            if (pinnum % 4) == 0 and pinnum <= wsnum:
                time.sleep(600)
